...
endpoint = r'''// POST /api/notify/ride-assigned
...
path = os.path.join(notifydir, 'ride-assigned.js')
open(path, 'w').write(endpoint)
print("1. Created api-server/api/notify/ride-assigned.js")

cmds = [
    'rm -f part3d_5_notify_endpoint.py',
    'git add api-server/api/notify/ride-assigned.js',
    'git commit -m "feat: ride-assigned notification endpoint (SMS+email to patient on accept) (Part 3D)"',
