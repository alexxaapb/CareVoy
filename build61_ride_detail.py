...
# ════════════════════════════════════════════════════════════════
# 1. CREATE app/ride/[id].tsx — the ride detail screen
# ════════════════════════════════════════════════════════════════
ride_dir = os.path.join(APP, 'app', 'ride')
os.makedirs(ride_dir, exist_ok=True)

detail_screen = r'''import { useLocalSearchParams, useRouter } from "expo-router";
...
detail_path = os.path.join(ride_dir, '[id].tsx')
open(detail_path, 'w').write(detail_screen)
print("1. Created app/ride/[id].tsx (ride detail + driver info + safety button)")

# ════════════════════════════════════════════════════════════════
# 2. Register the route in _layout.tsx
# ════════════════════════════════════════════════════════════════
lf = os.path.join(APP, 'app', '_layout.tsx')
lc = open(lf).read()

old_routes = '        <Stack.Screen name="care/add" options={{ headerShown: false }} />'
new_routes = '        <Stack.Screen name="care/add" options={{ headerShown: false }} />\n        <Stack.Screen name="ride/[id]" options={{ headerShown: false }} />'

if 'ride/[id]' not in lc:
    if old_routes in lc:
        lc = lc.replace(old_routes, new_routes)
        open(lf, 'w').write(lc)
        print("2. Registered ride/[id] route in _layout.tsx")
    else:
        print("2. FAILED to find route anchor")
else:
    print("2. Route already registered")
...
a = json.load(open(aj))
a['expo']['ios']['buildNumber'] = '61'
json.dump(a, open(aj, 'w'), indent=2)
print("3. Build number -> 61")

cmds = [
    'rm -f build61_ride_detail.py',
    'git add "artifacts/carevoy/app/ride/[id].tsx" artifacts/carevoy/app/_layout.tsx artifacts/carevoy/app.json',
    'git commit -m "feat: ride detail screen with driver info, status, safety button (build 61)"',
